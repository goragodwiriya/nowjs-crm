...
class Controller extends ApiController
{
    ...
    private function getSettingsResponse(Request $request, array $data, array $options = [], string $message = '')
    {
        ...
        $demo_mode = !$isSuperAdmin && !ApiController::isNotDemoMode($login);
        if ($demo_mode && in_array($message, ['API', 'SMS', 'LINE', 'Telegram'])) {
            return $this->errorResponse('Not available in demo mode', 403);
        }

        // Authorization:
        // - SuperAdmin: always allowed
        // - Admin: allowed to view settings
        // - Others: must have can_config to view
        if (!$isSuperAdmin && !ApiController::isAdmin($login) && !ApiController::hasPermission($login, 'can_config')) {
    ...
    private function getGeneralData()
    {
        ...
        if (file_exists(ROOT_PATH.DATA_FOLDER.'images/logo'.self::$cfg->stored_img_type)) {
            $result['logo'] = [
                [
                    'url' => WEB_URL.DATA_FOLDER.'images/logo'.self::$cfg->stored_img_type,
                    'name' => 'logo'
                ]
            ];
        } else {
    ...
    public function save(Request $request)
    {
        try {
            ...
            if (Config::save($config, ROOT_PATH.'settings/config.php')) {
                // Log
                \Index\Log\Model::add(0, 'Index', 'Save '.str_replace('-', ' ', $module).' Settings', $login->id);

                // Reload page
                return $this->redirectResponse('reload', 'Saved successfully', 200, 3000);
            }
    ...
    public function testEmail(Request $request)
    {
        try {
            ...
            // Log
            \Index\Log\Model::add(0, 'Index', 'Test email sent to '.$toEmail, $login->id);
    ...
    private function imageUpload(Request $request)
    {
        ...
        foreach ($request->getUploadedFiles() as $item => $file) {
            if (in_array($item, ['logo', 'bg_image'])) {
    ...
    private function removeImage(Request $request, $item)
    {
        try {
            ...
            if (file_exists($filePath)) {
                ...
                // Log the action
                \Index\Log\Model::add(0, 'Index', 'Removed '.$item.' image', $login->id);

                return $this->redirectResponse('reload', ucfirst(str_replace('_', ' ', $item)).' removed successfully');
            }

            // File doesn't exist, still success (idempotent)
            return $this->redirectResponse('reload', ucfirst(str_replace('_', ' ', $item)).' already removed');
        } catch (\Kotchasan\ApiException $e) {
            return $this->errorResponse($e->getMessage(), $e->getCode() ?: 400);
        } catch (\Exception $e) {
