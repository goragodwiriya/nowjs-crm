...
class Controller extends ApiController
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            $intendedUrl = $request->post('intended_url')->toString();

            // Authenticate user
            $result = Model::authenticate($username, $password, $clientIp);
    ...
    public function logout(Request $request)
    {
        try {
            ...
            if ($accessToken) {
                // Verify and get user ID for logging
    ...
    public function refresh(Request $request)
    {
        try {
            ...
            // Refresh the token
            $result = Model::refreshAccessToken($refreshToken);
    ...
    public function me(Request $request)
    {
        try {
            ...
            $img = DATA_FOLDER.'avatar/'.$login->id.self::$cfg->stored_img_type;
    ...
    /**
     * GET /index/auth/editprofile
     * Retrieve user profile for editing.
     *
     * @param Request $request
     *
     * @return mixed
    ...
    public function editprofile(Request $request)
    {
        try {
            ...
            $userData = Model::getEditProfile($userId);
    ...
    public function forgot(Request $request)
    {
        try {
            ...
            if (empty($identifier)) {
                return $this->errorResponse('Email or username is required', 400);
            }
    ...
    public function register(Request $request)
    {
        try {
            ...
            // Validate required fields
            if (empty($username)) {
                return $this->errorResponse('Username is required', 400);
            }
            ...
            if (in_array('email', self::$cfg->login_fields) && count(self::$cfg->login_fields) === 1 && !filter_var($username, FILTER_VALIDATE_EMAIL)) {
                return $this->errorResponse('Please enter a valid email address', 400);
            }
    ...
    public function settings(Request $request)
    {
        try {
            ...
            return $this->errorResponse('Failed to get settings', 500, $e);
        }
    }
