...
class Controller extends ApiController
{
    ...
    public function get(Request $request)
    {
        try {
            ...
            if (!$language) {
                return $this->redirectResponse('/404', 'Language not found', 404);
            }
    ...
    public function save(Request $request)
    {
        try {
            ...
            $save = [
                ...
                'type' => $type,
                'js' => $request->post('language_js')->toBoolean()
            ];
            ...
            // Installed language
            $languages = \Index\Language\Model::$languages;
            ...
            // Check for duplicate key with same js flag
            $db = \Kotchasan\DB::create();
            $existing = $db->first('language', [
                ['key', $save['key']],
                ['js', $save['js']]
            ]);

            if ($existing && $existing->id != $id) {
                return $this->formErrorResponse(['key' => 'Key already exists'], 400);
            }
            ...
            // Log
            \Index\Log\Model::add($savedId, 'Index', ($id === 0 ? 'Add' : 'Edit').' language: '.$save['key'], $login->id);
