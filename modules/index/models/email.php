...
class Model extends \Kotchasan\KBase
{
    ...
    /**
     * Send email with template - unified method for various modules
     *
     * @param array $params Email parameters:
     *   - to|email: Recipient email
     *   - subject: Email subject
     *   - body: HTML body content
     *   - from: Sender email (optional, uses noreply_email)
    ...
    public static function sendTemplate(array $params)
    {
        try {
            ...
            $from = $params['from'] ?? self::$cfg->noreply_email ?? null;

            $mail = \Kotchasan\Email::send($to, $from, $subject, $body);
            if ($mail->error()) {
