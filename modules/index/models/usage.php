...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params)
    {
        ...
        if (!empty($params['source'])) {
            $where[] = ['O.source', $params['source']];
        }
        if (!empty($params['from'])) {
            $where[] = ['O.create_date', '>=', $params['from']];
        }
        if (!empty($params['to'])) {
            $where[] = ['O.create_date', '<=', $params['to']];
        }
    ...
    public static function getSourceOptions()
    {
        return static::createQuery()
            ->select(Sql::DISTINCT('source', 'text'), Sql::create('LOWER(source) value'))
            ->from('logs')
            ->CacheOn()
            ->execute()
            ->fetchAll();
    ...
    public static function remove($ids)
    {
        ...
        return \Kotchasan\DB::create()->delete('logs', [['id', $ids]]);
    }
}
