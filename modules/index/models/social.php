...
class Model extends \Kotchasan\Model
{
    ...
    private static function logSocialRegistration($userId, $provider)
    {
        try {
            if (class_exists('\\Index\\Log\\Model')) {
                ...
                \Index\Log\Model::add($userId, 'Auth', ucfirst($provider)." Registration provider={$provider} IP={$clientIp}", $userId);
            }
        } catch (\Exception $e) {
            // Silently fail
        }
    ...
    private static function logSocialLink($userId, $provider)
    {
        try {
            if (class_exists('\\Index\\Log\\Model')) {
                $clientIp = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
                \Index\Log\Model::add($userId, 'Auth', ucfirst($provider)." Linking provider={$provider} IP={$clientIp}", $userId);
            }
        } catch (\Exception $e) {
            // Silently fail
        }
