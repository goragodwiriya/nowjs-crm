...
class ApiController extends \Kotchasan\KBase
{
    ...
    {
        $allowOrigin = !empty(self::$cfg->api_cors) ? self::$cfg->api_cors : '*';
        $allowHeaders = 'origin, x-requested-with, content-type, authorization, x-api-token, x-access-token, x-csrf-token';
    ...
    /**
     * Initialize language from Accept-Language header
     *
    ...
    {
        $acceptLanguage = $request->server('HTTP_ACCEPT_LANGUAGE')->toString();
        $lang = !empty($acceptLanguage) ? substr($acceptLanguage, 0, 2) : 'en';
     ...
     * @return Response
     */
    protected function redirectResponse($url, $message = '', $code = 200, $delay = 0)
    {
        $actions = [
            [
                'type' => 'redirect',
    ...
    protected function authenticateRequest(Request $request)
    {
        ...
        $user = $db->first('user', [['token', $accessToken]]);
        if ($user) {
        ...
        if (!empty(self::$cfg->jwt_secret)) {
            try {
                ...
                if (!empty($payload) && isset($payload['sub'])) {
                    ...
                    if ($jwtUserId > 0) {
                        $jwtUser = $db->first('user', [['id', $jwtUserId]]);
                        if ($jwtUser) {
                            return $jwtUser;
                        }
                    }
    ...
    protected function getResponseHeaders(): array
    {
        $headers = [
            ...
            'Access-Control-Allow-Headers' => 'Content-Type, Authorization, X-API-Token, X-Access-Token, X-CSRF-TOKEN',
