...
if (defined('ROOT_PATH')) {
    ...
    try {
        ...
        echo '<p><a href="index.php?step=2" class="btn large btn-secondary">กลับไปลองใหม่</a></p>';
    }
    if (!$error) {
        ...
        if (!$error) {
            try {
                ...
                $sql .= "(3, 'saksom@hotmail.com', '$salt', '".sha1($password_key.'saksom'.$salt)."', NULL, 0, '', 'Saksom', '$today');";
                $sql .= "(4, 'aoch@hotmail.com', '$salt', '".sha1($password_key.'aoch'.$salt)."', NULL, 0, '', 'Aoch', '$today');";
                $sql .= "(5, 'book@hotmail.com', '$salt', '".sha1($password_key.'book'.$salt)."', NULL, 0, '', 'Book', '$today');";
                $db->query($sql);
            } catch (\PDOException $ex) {
                $error = true;
                $content[] = '<li class="incorrect">'.$ex->getMessage().'</li>';
